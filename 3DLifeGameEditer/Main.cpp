...
LifeGameInfo setLifeGame( Array<_Triangle3D> cubePolygons,Object core) {
	Vec3 pos;
		...
		for (int j = 0; j < SIDE_CELLS; j++) {
			...
			for (int k = 0; k < SIDE_CELLS; k++) {
				...
				if (rand() % 10000 <= CELL_PER*100) {
					models << Model{ putModel(cubePolygons,pos), core, { i,j,k }, 100 };
					fieldState[i][j] &= 1 << k;
				}
			}
		}
